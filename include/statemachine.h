...
struct MovementData : public EventData
{
public:
	MovementData();
	int					mIsFalling;
...
protected:
    enum { EVENT_IGNORE = 0xFE, CANNOT_HAPPEN };
    unsigned char	mCurrentState;
    void			ExternalEvent(unsigned char, EventData* = NULL);
    void			InternalEvent(unsigned char, EventData* = NULL);
    virtual const	StateStruct* GetStateMap() = 0;
	virtual const	StateStructEx*  GetStateMapEx() = 0; 
private:
    const							int mMaxStates;
    bool							mEventGenerated;
    EventData*						mEventData;
    void							StateEngine(void);
	std::queue<unsigned char>		mPrevStates;
};
 
typedef void (StateMachine::*StateFunc)(EventData* n);
struct StateStruct 
{
...
#define END_TRANSITION_MAP(data) \
    0 };\
    ExternalEvent(TRANSITIONS[mCurrentState], data);


class Movement : public StateMachine
{
public:
	Movement(): StateMachine(MAX_STATES){};
	void Idle(MovementData* n);
	void Move(MovementData* n);
	void Jump(MovementData* n);

private:
	void		ST_Idle(MovementData* n);
	void		ST_Move(MovementData* n);
	void		ST_Jump(MovementData* n);

	//STATE_DECLARE(Movement, Idle, NoEventData);
	
	// state map to define state function order
    BEGIN_STATE_MAP
		STATE_MAP_ENTRY(&Movement::ST_Idle)
        STATE_MAP_ENTRY(&Movement::ST_Move)
        STATE_MAP_ENTRY(&Movement::ST_Jump)
    END_STATE_MAP
